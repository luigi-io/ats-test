...
export class BatchBurnCommandHandler implements ICommandHandler<BatchBurnCommand> {
  constructor(
  ...
  async execute(command: BatchBurnCommand): Promise<BatchBurnResponse> {
    try {
      ...
      const res = await handler.batchBurn(securityEvmAddress, bdList, evmAddresses, securityId);

      return Promise.resolve(new BatchBurnResponse(res.error === undefined, res.id!));
    } catch (error) {

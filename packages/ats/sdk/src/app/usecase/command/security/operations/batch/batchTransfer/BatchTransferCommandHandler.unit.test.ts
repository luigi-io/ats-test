...
import { BatchTransferCommand, BatchTransferResponse } from "./BatchTransferCommand";
...
describe("BatchTransferCommandHandler", () => {
  ...
  describe("execute", () => {
    it("throws BatchTransferCommandError when command fails with uncaught error", async () => {
      ...
      await expect(resultPromise).rejects.toBeInstanceOf(BatchTransferCommandError);
      await expect(resultPromise).rejects.toMatchObject({
        message: expect.stringContaining(`An error occurred while batch transfer: ${errorMsg}`),
    ...
    it("should successfully batch freeze partial tokens", async () => {
      ...
      expect(transactionServiceMock.getHandler().batchTransfer).toHaveBeenCalledTimes(1);

      expect(validationServiceMock.checkPause).toHaveBeenCalledWith(command.securityId);
      expect(contractServiceMock.getContractEvmAddress).toHaveBeenCalledWith(command.securityId);

      expect(transactionServiceMock.getHandler().batchTransfer).toHaveBeenCalledWith(
        evmAddress,

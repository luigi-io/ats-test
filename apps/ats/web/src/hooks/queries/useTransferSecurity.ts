...
export const useTransferSecurity = (options: UseMutationOptions<boolean, unknown, TransferRequest, unknown> = {}) => {
  ...
  //@ts-ignore
  return useMutation((transferRequest: TransferRequest) => SDKService.transfer(transferRequest), {
    onSuccess: (data) => {
      if (data === true) {
        toast.show({
          duration: 3000,

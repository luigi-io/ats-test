...
export const useUnpauseSecurity = (options: UseMutationOptions<boolean, unknown, PauseRequest, unknown> = {}) => {
  const toast = useToast();
  const { t } = useTranslation("security", { keyPrefix: "unpause" });

  return useMutation((pauseRequest: PauseRequest) => SDKService.unpause(pauseRequest), {
    onSuccess: (data) => {
      if (data === true) {
        toast.show({
          duration: 3000,
